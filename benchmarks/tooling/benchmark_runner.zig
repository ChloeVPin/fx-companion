...
fn runPair(root: []const u8, stock_first: bool) !Pair {
    ...
    var stock: Timed = undefined;
    var cold: Timed = undefined;
    var boosted: Timed = undefined;
    if (stock_first) {
        stock = try discover(stock_arena.allocator(), root, false);
        cold = try discover(cold_arena.allocator(), root, true);
        boosted = try discover(warm_arena.allocator(), root, true);
    } else {
        cold = try discover(cold_arena.allocator(), root, true);
        boosted = try discover(warm_arena.allocator(), root, true);
        stock = try discover(stock_arena.allocator(), root, false);
...
pub fn run(root: []const u8) !void {
    ...
    for (0..rounds) |i| {
        const stock_first = i % 2 == 0;
        const sample = try runPair(root, stock_first);
        stock_times[i] = sample.stock;
